// ==============================
// Beeferino – vokabeln.js (Finale Version)
// ==============================
...
function renderTable() {
  ...
  document.getElementById("countDesk").textContent =
    `📘 ${data.length} Vokabel${data.length !== 1 ? "n" : ""} gefunden`;
...
function enableSorting() {
  document.querySelectorAll("th.sortable").forEach((th) => {
    th.onclick = () => {
      ...
      list.sort(
        (a, b) =>
          (a[col] || "").toLowerCase().localeCompare((b[col] || "").toLowerCase()) * sortState.dir
      );
      saveLocal(list);
      renderTable();
    };
...
// === Modal ===
// (modal code hier bleibt unverändert – Editor bleibt gleich)
...
function bulkDelete() {
  const checks = Array.from(document.querySelectorAll(".rowcheck:checked")).map(
    (x) => +x.dataset.idx
  );
  if (!checks.length) {
...
function renderMobileView() {
  ...
  app.innerHTML = `
    <div class="mobile-app">
      <h2 class="mob-header">Vokabeln</h2>
      <div id="mobList" class="mob-list"></div>

      <div class="floating-menu">
        <button id="themeToggle" class="float-btn small" title="Theme wechseln">☀️</button>
        <button id="abcToggle" class="float-btn blue" title="Alphabet-Filter">A–Z</button>
        <button id="addMob" class="float-btn green" title="Neue Vokabel">＋</button>
      </div>
    </div>
  `;
...
function renderMobileList() {
  ...
  container.innerHTML = list
    .map(
      (v, i) => `
      <div class="mob-card" data-idx="${i}" style="border-top:5px solid ${COLOR_MAP[v[2]]}">
        <div class="mob-card-top" style="color:${TEXT_ON[v[2]]};font-weight:bold">
          ${CATEGORY_NAMES[v[2]]}
        </div>
        <div class="mob-card-content">
          ...
          ${
            v[6] === 1
              ? `<div class="mob-warning">⚠️ Verwechslungsgefahr</div>`
              : ""
          }
        </div>
      </div>`
...
// === Startlogik: Automatische Umschaltung ===
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("resize", () => {
    ...
    if (nowMobile && !isCurrentlyMobile) {
      document.body.classList.add("mobile-mode");
      renderMobileView();
    } else if (!nowMobile && isCurrentlyMobile) {
      document.body.classList.remove("mobile-mode");
      loadData();
    }
  });
